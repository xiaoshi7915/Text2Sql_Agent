...
class LLMService:
    ...
    def __init__(self):
        ...
        # 初始化客户端
        self.client = None
        if self.llm_provider == 'anthropic' and self.anthropic_api_key and ANTHROPIC_AVAILABLE:
...
这个查询将从users表中检索所有活跃用户的信息，并限制返回1000条记录。"""
    ...
    def natural_language_to_sql(self, query, metadata, sample_data=None, conversation_history=None):
        ...
        try:
            ...
            if (self.llm_provider == 'anthropic' and not self.anthropic_api_key) or \
               (self.llm_provider == 'deepseek' and not self.deepseek_api_key):
